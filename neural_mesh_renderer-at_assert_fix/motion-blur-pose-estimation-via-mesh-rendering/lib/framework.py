...
class Framework(imagegeneration.ImageGeneration, nn.Module):
	def __init__(self, cam_index, img_ref, img_cur, t_ref, t_cur, pointcloud_ray, faces, dist_tran_norm, dist_angl_norm, cur_quat, cur_tran_SE3, N_poses, pyramid_scale):
		...
		dist_direction_tran = 3 # [x = 1, -x = 2, y = 3, -y = 4, z = 5, -z = 6]
		...
		# self.cur_pose_se3 = nn.Parameter(torch.tensor([-1.2634,  1.0392,  0.8188,  0.0450,  0.1454, -0.6075]), requires_grad = True)
	...
	def VGG19(self):
		# testing convolutional neural network preprocessing
		conv_3_3_layer = 14
	...
	def forward(self):
		...
		# L2 or L1 norm
		# loss = torch.sum((blur_image - self.blur_ref) * (blur_image - self.blur_ref))
		loss = torch.sum(torch.abs(blur_image - self.blur_ref))

...
class Framework(imagegeneration.ImageGeneration, nn.Module):
	def __init__(self, cam_index, img_ref, img_cur, t_ref, t_cur, pointcloud_ray, faces, dist_tran_norm, dist_angl_norm, cur_quat, cur_tran_SE3, N_poses, pyramid_scale):
		...
		# self.cur_pose_se3 = nn.Parameter(torch.tensor([-1.2634,  1.0392,  0.8188,  0.0450,  0.1454, -0.6075]), requires_grad = True) # quick solution for coarse-to-sparse

		# reading in blurry image and converting it to torch tensor
		blur_obj = dataset.Blur()
		blur_ref = blur_obj.get_scaled_blur_image(self.cam_index, self.img_cur, self.pyramid_scale)
		self.blur_ref = torch.tensor(blur_ref).float()

		self.img_saving = 0 # parameter for image saving

	# trying out CNN preprocessing
	# def VGG19(self):
	# 	conv_3_3_layer = 14
	...
	# forward call
	def forward(self):
		blur_image = self.blurrer(self.cam_index, self.img_ref, self.img_cur, self.t_ref, self.t_cur, self.pointcloud_ray, self.faces, self.cur_pose_se3, self.N_poses, self.pyramid_scale) # calling the blurrer
		...
		# objective function
		# loss = torch.sum((blur_image - self.blur_ref) * (blur_image - self.blur_ref)) # L2-norm
		loss = torch.sum(torch.abs(blur_image - self.blur_ref)) # L1-norm

		return loss # return current loss


# just for generating images
class Framework_image_generator(imagegeneration.ImageGeneration):
	def __init__(self, cam_index, img_ref, img_cur, t_ref, t_cur, pointcloud_ray, faces, cur_quat, cur_tran_SE3, sharp, N_poses, pyramid_scale):
		...
		depth = self.render_depth(self.cam_index, self.pointcloud_ray, self.faces, self.cur_pose_se3, self.pyramid_scale) # generating depth image at that pose

		if sharp == True:
			image = self.reprojector(self.cam_index, self.img_ref, self.t_ref, depth, self.cur_pose_se3, self.pyramid_scale) # generate image at that pose
		else:
			image = self.blurrer(self.cam_index, self.img_ref, self.img_cur, self.t_ref, self.t_cur, self.pointcloud_ray, self.faces, self.cur_pose_se3, self.N_poses, self.pyramid_scale) # calling the blurrer
